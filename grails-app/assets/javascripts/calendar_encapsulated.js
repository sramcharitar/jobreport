...
var calendarPlugin = (function(){
	...
	var dictionaryFr = { today: 'Aujourd\'hui' , close : 'Fermer' , closeCalendar: 'Fermeture du calendrier' , disabled: 'désactivé' , nextMonth: 'Mois suivant ALT+N' , previousMonth: 'Mois précédent ALT+P' , nextYear: 'Année suivante ALT+Y' , previousYear: 'Année précédente ALT+L', selCurDate: 'Date courante' };
	var dictionaryEn = { today: 'Today', close : 'Close' , closeCalendar: 'Close Calendar' , disabled: 'disabled' , nextMonth: 'Next Month ALT+N' , previousMonth: 'Previous Month ALT+P' , nextYear: 'Next Year ALT+Y' , previousYear: 'Previous Year ALT+L', selCurDate: 'Current date' };
	...
	var pastDateEn = "Past Date";
	var pastDateFr = "Date pass&eacute;e";

	var startEnd = startEndEN;

	var currDate = currDateEn;
	var futDate = futDateEn;
	var pastDAte = pastDateEn;
	...
	function createPopupCalendar( from, options )
	{
		...
		popupCalendar = $('<div class="calendar-wrapper-popup"><div class="calendar-header"><div class="calendar-header-close"><a href="#" onclick="return calendarPlugin.dismissPopupCalendar();" class="linkedtextandicon"><span>' + dictionary['close'] + '</span> <img src="/uos/_assets/images/icons/close.gif" width="16" height="16" alt="' + dictionary['closeCalendar'] + '"></a></div><div class="calendar-header-today"><a href="#" title="Today is December 10, 2009">Today</a> is December 10, 2009</div></div></div>');
		...
		popupCalendar.append( '<div class="calendar-footer"><div class="calendar-footer-close"><a href="#" onclick="return calendarPlugin.dismissPopupCalendar();" class="linkedtextandicon footerClose"><span>' + dictionary['close'] + '</span> <img src="/uos/_assets/images/icons/close.gif" width="16" height="16" alt="' + dictionary['closeCalendar'] + '"></a></div></div>' );
		
		var footerClose = popupCalendar.find('.footerClose')[0];
		
		//footerClose.addEventListener('keydown', (function (e){
		footerClose.onkeydown = (function (e){
			var event = window.event ? window.event : e;
			var isIE = event.target ? false : true;
			
			if(event.keyCode == 13) return dismissPopupCalendar(); // Added on 30 October 2014
			if(event.keyCode != 9) return false;
			
			var prev = popupCalendar.find('.calendar-nav-previousmonth')[0];
			var next = popupCalendar.find('.calendar-nav-nextmonth')[0]; 
	...
	function __populateHeader( obj , year, month )
	{
		...
		$('#selCurDateLink')[0].innerHTML = $('#selCurDateLink')[0].text = dictionary['selCurDate'];
		$('#selCurDateLink').click( function() { return selectDate( obj , new Date()) } );
	...
	function __populateCalendar( obj, year, month )
	{
		...
		switch(allow[0]){
			...
			case 'sundays':
				...
				break;
			case 'custom':
				allow = [options['customAllowFilter']];
				break;
			default:
				allow = [everyday];
		}
		...
		for( var i = 0 ; i < 6 ; i++ )
		{
			for( var j = 0 ; j < 7 ; j++ )
			{
				...
				else			
				{
					...
					{
						var cellDate = new Date(day.getFullYear(), day.getMonth(), day.getDate());
						var today = new Date();
						...
						if(cellDate > today) {
							...
							var theDateToFillInTitle = " (" + dayNames[day.getDay()] + ", " + monthNames[day.getMonth()] + " " + day.getFullYear() + ") - " + pastDate;
							var theDateToFillIn = " " + pastDate;
						}
						else {
	...
	return {
			"createPopupCalendar" : function ( from, options ) {
				return createPopupCalendar(from, options);
			},
			
			"createInlineCalendar" : function ( obj , options ){
				createInlineCalendar( obj , options )
			},
			
			"createInlineDoubleCalendar" : function ( a, b , options ){
				createInlineDoubleCalendar( a, b , options );
			},
			
			"createPopupDoubleCalendar" : function ( from, options ){
				createPopupDoubleCalendar( from, options );
			},
			
			"dismissPopupCalendar" : function () {
				dismissPopupCalendar();
				return false;  //Addition on 30 October 2014
			},
			...
			"mondays" : function( date ){
				mondays( date );
			},
			
			"tuesdays" : function( date ){
				tuesdays( date );
			},
			
			"wednesdays" : function( date ){
				wednesdays( date );
			},
			
			"thursdays" : function( date ){
				thursdays( date );
			},
			...
			"everyday" : function( date ){
				everyday( date );
			},
			
			"selectDate" : function ( obj , date ){
				selectDate( obj , date );
			}			
		};
